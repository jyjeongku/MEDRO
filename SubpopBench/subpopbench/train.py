...
if __name__ == "__main__":
    ...
    parser.add_argument('--es_strategy', choices=['metric', 'oracle_metric'], default='metric')
    ...
    if args.algorithm == 'DFR' or args.algorithm == 'MEDRO_gate':
    ...
    if args.algorithm == 'MEDRO_gate':
        args.pretrained = os.path.join(args.output_dir.replace(args.output_folder_name, args.stage1_folder),
                                       hparams['stage1_model']).replace(args.algorithm, args.stage1_algo)
        args.pretrained = args.pretrained.replace(f"seed{args.pretrained[args.pretrained.find('seed') + len('seed')]}", f"seed{args.seed}")
        assert os.path.isfile(args.pretrained)
        checkpoint = torch.load(args.pretrained, map_location="cpu")
        
        stage1_algorithm_class = algorithms.get_algorithm_class(args.stage1_algo)
        stage1_algorithm = stage1_algorithm_class(data_type,
                                                  input_shape,
                                                  num_labels,
                                                  num_attributes,
                                                  len(train_dataset),
        ...
        stage1_algorithm.load_state_dict(checkpoint['model_dict'])
        import copy
        algorithm.featurizer = copy.deepcopy(stage1_algorithm.featurizer)
        algorithm.load_multi_head_network(stage1_algorithm.network.heads)
        print(f"===> Stage1 model loaded for MEDRO_gate: '{args.pretrained}'")
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            #
            full_test_metrics = curr_metrics['te']
            ...
            if args.use_es:
                ...
                elif args.es_strategy == 'oracle_metric':
                    es_metric_val = full_test_metrics[es_group][es_metric]
