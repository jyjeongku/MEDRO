...
class MultiHeadModel(nn.Module):
    def __init__(self, featurizer, heads, inference_mode="mean"):
        super().__init__()
        ...
        if isinstance(heads, list):
            ...
            raise ValueError("heads must be a list or nn.ModuleList")
    ...
    def forward(self, x):
        ...
        for head_module in self.heads:
            out = head_module(feats)
            all_logits.append(out.unsqueeze(0))
            
        all_logits = torch.cat(all_logits, dim=0)
        
        if self.inference_mode == "mean":
            return all_logits.mean(dim=0)
        elif self.inference_mode == "all":
            return all_logits
        else:
            raise NotImplementedError(f"inference_mode={self.inference_mode} not supported.")