...
class MEDRO_default(ERM):
    def __init__(self, data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes=None):
        super(MEDRO_default, self).__init__(data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes)
        
        self.num_groups = self.num_classes * self.num_attributes
        
        heads_list = []
        for _ in range(self.num_groups):
            head_i = networks.Classifier(in_features=self.featurizer.n_outputs,
                                         out_features=self.num_classes,
                                         is_nonlinear=self.hparams['nonlinear_classifier'])
            heads_list.append(head_i)
        
        self.network = networks.MultiHeadModel(featurizer=self.featurizer,
                                               heads=heads_list,
                                               inference_mode="all")
        
        MEDRO_weight_num = self.num_groups * self.num_groups
        self.register_buffer("MEDRO_weights", torch.ones(MEDRO_weight_num).cuda() / float(MEDRO_weight_num))
    ...
    def _compute_loss(self, i, x, y, a, step):
        ...
        per_sample_losses = []
        for i_head in range(self.num_groups):
            logits_i = all_logits[i_head]
            loss_i = self.loss(logits_i, y)
            per_sample_losses.append(loss_i.unsqueeze(0))
        
        per_sample_losses = torch.cat(per_sample_losses, dim=0)
        risk_list, total_expert_loss = self._compute_risk_list(per_sample_losses, y, a)
        dro_loss = self.compute_robust_loss_allrisks(risk_list)
        
        total_loss = total_expert_loss + dro_loss
        return total_loss
    
    def _compute_risk_list(self, per_sample_losses, y, a):
        group_idx = y * self.num_attributes + a
        risk_list_out = []
        total_expert = 0.0
        
        for i_head in range(self.num_groups):
            losses_i = per_sample_losses[i_head]
            group_loss_i = torch.zeros(self.num_groups, device=losses_i.device)
            group_count_i = torch.zeros(self.num_groups, device=losses_i.device)
            
            unique_g = group_idx.unique()
            for g_idx in unique_g:
                mask = (group_idx == g_idx)
                if mask.any():
                    group_loss_i[g_idx] = losses_i[mask].mean()
                    group_count_i[g_idx] = float(mask.sum())
                    tmp_group_loss_i = group_loss_i[i_head].clone()
            total_expert += tmp_group_loss_i
            risk_list_out.append(group_loss_i)
        
        total_expert /= float(self.num_groups)
        risk_list_out = torch.cat(risk_list_out, dim=0)
        
        return risk_list_out, total_expert
    
    def compute_robust_loss_allrisks(self, group_loss_vector):
        """
        group_loss_vector & MEDRO_weight shape --> [MEDRO_weight_num, ]
        """
        step_size = self.hparams["MEDRO_eta"]
        
        self.MEDRO_weights = (self.MEDRO_weights * torch.exp(step_size * group_loss_vector)).detach()
        self.MEDRO_weights /= self.MEDRO_weights.sum()
        dro_loss = (group_loss_vector * self.MEDRO_weights).sum()
        return dro_loss
    ...
    def predict(self, x):
        tmp_mode = self.network.inference_mode
        self.network.inference_mode = "mean"
        out = self.network(x)
        self.network.inference_mode = tmp_mode
        return out
    
class MEDRO(MEDRO_default):
    def __init__(self, data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes=None):
        super(MEDRO, self).__init__(data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes)
        
        self.register_buffer("MEDRO_weights_matrix", self.MEDRO_weights.view(self.num_groups, self.num_groups))
        
    def _compute_loss(self, i, x, y, a, step):
        all_logits = self.network(x)
        per_sample_losses = torch.stack([self.loss(logits, y) for logits in all_logits])
        ...
        group_onehot.scatter_(1, group_idx.unsqueeze(1), 1.0)
        
        risk_sums = torch.einsum('hb,bg->hg', per_sample_losses, group_onehot)
        group_count = group_onehot.sum(0, keepdim=True) # [1, num_groups]
        
        risk_matrix = torch.zeros_like(risk_sums)
        valid_mask = group_count > 0
        valid_mask = valid_mask.view(-1)
        risk_matrix[:, valid_mask] = risk_sums[:, valid_mask] / group_count[:, valid_mask]
        total_expert = torch.diag(risk_matrix).sum()
        
        step_size = self.hparams["groupdro_eta"]
        self.MEDRO_weights_matrix = (self.MEDRO_weights_matrix * torch.exp(step_size * risk_matrix)).detach()
        self.MEDRO_weights_matrix /= self.MEDRO_weights_matrix.sum()
        dro_loss = (risk_matrix * self.MEDRO_weights_matrix).sum()
        
        return (total_expert / valid_mask.sum()) + dro_loss

class MEDRO_gate(ERM):
    """
    MEDRO with a gating mechanism for the second stage trainig.
    Freezes feature extractor and multi-heads from first stage, and only trains a simple gating layer.
    """
    def __init__(self, data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes=None):
        super(MEDRO_gate, self).__init__(
            data_type, input_shape, num_classes, num_attributes, num_examples, hparams, grp_sizes)
        
        for name, param in self.featurizer.named_parameters():
            param.requires_grad = False
        
        num_groups = num_classes * num_attributes
        self.gate_layer = nn.Linear(self.featurizer.n_outputs, num_groups)
    ...
    def load_multi_head_network(self, network):
        for name, param in self.featurizer.named_parameters():
            param.requires_grad = False
        self.multi_head_network = network
        
        for param in self.multi_head_network.parameters():
            param.requires_grad = False
    ...
    def predict(self, x):
        if self.multi_head_network is None:
            raise ValueError ("Multi_head_network has not been loaded.")
        
        with torch.no_grad():
            features = self.featurizer(x)
            all_logits = []
            for head in self.multi_head_network:
                logits = head(features)
                all_logits.append(logits)
            all_logits = torch.stack(all_logits)
            
        # compute gate weights
        gate_logits = self.gate_layer(features)
        gate_weights = F.softmax(gate_logits, dim=1)
        # batch mat.mul.
        gate_weights = gate_weights.unsqueeze(1) # [batch_size, 1, num_heads]
        all_logits = all_logits.permute(1, 0, 2) # [batch_size, num_heads, num_classes]
        weighted_logits = torch.bmm(gate_weights, all_logits).squeeze(1) # [batch_size, num_classes]
